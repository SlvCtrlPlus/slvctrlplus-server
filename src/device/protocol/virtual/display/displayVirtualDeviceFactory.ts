import DisplayVirtualDevice from "./displayVirtualDevice.js";
...
export default class DisplayVirtualDeviceFactory implements VirtualDeviceFactory
{
    ...
    public async create(knownDevice: KnownDevice, provider: string): Promise<Device> {
        return new Promise<Device>((resolve) => {
            const device = new DisplayVirtualDevice(
                "1.0.0",
                knownDevice.id,
                knownDevice.name,
                knownDevice.type,
    ...
    public forDeviceType(): string {
        return DeviceDiscriminator.fromValue(DisplayVirtualDevice);
    }
}
