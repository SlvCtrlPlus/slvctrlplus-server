...
describe('deviceManager', () => {
    ...
    describe('reset', () => {
        ...
        it('closes every connected device', async () => {
            const deviceId = DeviceId.create('device-reset-1');
            const deviceInfo: DeviceDetectionInfo = { type: 'test', detectionId: deviceId };
            const manager = new DeviceManager(mockedEventEmitter, new Map(), mockedSettingsManager, mockedLogger);
        ...
        it('rejects every pending offer in the queue with a DeviceOfferRejectedError', async () => {
            const deviceId = DeviceId.create('device-reset-2');
            const deviceInfo: DeviceDetectionInfo = { type: 'test', detectionId: deviceId };
            const manager = new DeviceManager(mockedEventEmitter, new Map(), mockedSettingsManager, mockedLogger);

            manager.announceDetectedDevice(deviceInfo);

            // First offer never settles on its own, so both offers are still queued when reset() runs
            const firstPendingPromise = manager.offerDevice(deviceInfo, () => new Promise<AnyDevice | undefined>(() => {}));
            const secondPendingPromise = manager.offerDevice(deviceInfo, () => Promise.reject(new Error('should never run')));

            await manager.reset();

            const [firstResult, secondResult] = await Promise.all([firstPendingPromise, secondPendingPromise]);
        ...
        it('allows the same detection id to be announced again afterwards', async () => {
            const deviceId = DeviceId.create('device-reset-3');
            const deviceInfo: DeviceDetectionInfo = { type: 'test', detectionId: deviceId };
            const manager = new DeviceManager(mockedEventEmitter, new Map(), mockedSettingsManager, mockedLogger);
        ...
        it('clears pending retries so a re-enable afterwards does not resurrect them', async () => {
            const deviceId = DeviceId.create('device-reset-4');
            const deviceInfo: DeviceDetectionInfo = { type: 'test', detectionId: deviceId };
            ...
            manager.announceDetectedDevice(deviceInfo);
            const device = new TestDevice(deviceId, 'Foo', new Date(), false, new EventEmitter());
            await manager.offerDevice(deviceInfo, () => Promise.resolve(device)); // rejected: disabled, parked as pending retry
        ...
        it('still closes remaining devices and throws when closing one of them fails', async () => {
            const deviceIdA = DeviceId.create('device-reset-5a');
            const deviceIdB = DeviceId.create('device-reset-5b');
            const deviceInfoA: DeviceDetectionInfo = { type: 'test', detectionId: deviceIdA };
            const deviceInfoB: DeviceDetectionInfo = { type: 'test', detectionId: deviceIdB };

            const manager = new DeviceManager(mockedEventEmitter, new Map(), mockedSettingsManager, mockedLogger);

            const deviceA = new TestDevice(deviceIdA, 'Foo', new Date(), false, new EventEmitter());
            const closeError = new Error('failed to close');
            vi.spyOn(deviceA, 'close').mockRejectedValue(closeError);

            const deviceB = new TestDevice(deviceIdB, 'Bar', new Date(), false, new EventEmitter());
            const closeSpyB = vi.spyOn(deviceB, 'close');

            await connectDevice(manager, deviceInfoA, deviceA);
            await connectDevice(manager, deviceInfoB, deviceB);

            await expect(manager.reset()).rejects.toBe(closeError);
