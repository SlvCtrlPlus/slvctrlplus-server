...
TypeScript:
- Never use type assertions (i.e. `as`), always type things properly - If there's no way around it: ask
- Never use `any` as a type, always type things properly - If there's no way around it: ask

Git:
- A git branch name has to follow the pattern `^(feat|chore|fix)/(.+)$`
- If you create a new git branch: ask for the branch name
